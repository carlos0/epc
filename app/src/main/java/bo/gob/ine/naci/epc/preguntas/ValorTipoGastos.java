...
public class ValorTipoGastos extends PreguntaView {
    ...
    public ValorTipoGastos(final Context context, final int posicion, final int id, int idSeccion, String cod, String preg, final Map<Integer, String> opciones, int maxLength, int buttonsActive, String ayuda, final String codEsp, final Boolean evaluar) {
...
//                    String val = String.format(Locale.US, "%.3f",Float.parseFloat(txtConvertido.getText().toString()));
                    if(!hayRespuesta){
                        resultado = Float.parseFloat(valor.getText().toString()) * conversor;
                    }
